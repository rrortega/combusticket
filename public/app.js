...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupEventListeners() {
    ...
    const PWA_DISMISSED_KEY = "combusticket_pwa_banner_dismissed";

    function showPwaBanner() {
      if (!pwaInstallBanner) return;
      if (sessionStorage.getItem(PWA_DISMISSED_KEY)) return;
      pwaInstallBanner.classList.remove("hidden", "dismissing");
    }

    function hidePwaBanner() {
      if (!pwaInstallBanner) return;
      pwaInstallBanner.classList.add("dismissing");
      setTimeout(() => pwaInstallBanner.classList.add("hidden"), 380);
    }
  ...
  /**
   * Returns an SVG icon representing the payment method.
   * Used in mobile meta strip to save horizontal space.
   */
